...
//        createIndexBuffer();
   ...
   void Cube::createSurface() {
    ...
    void Cube::createCommandPool() {
     ...
     void Cube::createBuffer(VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags properties, VkBuffer& buffer, VkDeviceMemory& bufferMemory) {
...
//            gravitySystem.update(gameObjects, 1.f / 60,5);
  ...
  //          vecFieldSystem.update(gravitySystem,gameObjects,particles);
    ...
    void Cube::renderObjects(VkCommandBuffer commandBuffer, std::vector<GameObject>& objects)
    {
            int i{};
            for (auto& obj : objects)
            {
                obj.transformCom.rotation.y = glm::mod(obj.transformCom.rotation.y + .01f, glm::two_pi<float>());
                SimplePushConstantData push{};
//                push.offset = obj.transformCom.translation;

                push.color = obj.color;
                push.transform = obj.transformCom.matCal();
   ...
   std::vector<const char*> Cube::getRequiredExtensions() {
    ...
    void Cube::loadGameObjects()
    {
      ...
      {{.5f, .5f, -0.5f}, {.1f, .8f, .1f}},
        ...
        cube.transformCom.scale = {0.5f, .5f,.5f};
